...
{
	int err = 0, ret;
	struct dentry *lower_dentry,  *lower_parent_dentry = NULL;
	struct path lower_path, saved_path;
	char *val = NULL, *value = NULL, *typ_val = NULL;
	int integ_size = MD5_SIGNATURE_SIZE, str_md5, str_sha1;
	...
	// Kmalloc value to store has_integrity	
	value = kmalloc( 2 , GFP_KERNEL );
	if (!value)
	{
	printk("Couldn't kmalloc value\n");
	err = -ENOMEM;
	goto out_unlock;
	}
	memset (val , 0 , 2);
	...
	// Get has_integrity for parent
	ret = vfs_getxattr(lower_parent_dentry, (char *) USR_HAS_INT, (void *) val, (size_t) 1);
	...
	if(ret == 1)
	{
		...
		// Set has_integrity
		ret = vfs_setxattr(lower_path.dentry, (char *) USR_HAS_INT, (void *) value, (ssize_t) 1, XATTR_CREATE);
		if (val[0] == '1')
		{
			// Blank MD5 store in file
				
			ret = vfs_setxattr(lower_path.dentry, (char *) USR_INT_VAL, (void *) BLANK_MD5, (ssize_t) 2*integ_size, XATTR_CREATE);
			...
			// Kmalloc val to store has_integrity	
			typ_val = kmalloc( 4 , GFP_KERNEL );
			if (!typ_val)
			{
			printk("Couldn't kmalloc typ_val\n");
			err = -ENOMEM;
			goto out_unlock;
			}
			memset (typ_val , 0 , 4);
				
			ret = vfs_getxattr(lower_parent_dentry, (char *) USR_INT_TYP, (void *) typ_val, (size_t) 3);
			
			if (ret == 3)
			{
				ret = vfs_setxattr(lower_path.dentry, (char *) USR_INT_TYP, (void *) typ_val, (ssize_t) 3, XATTR_CREATE);
				
					str_md5 = strcmp(typ_val, (char *) MD5_STR);
					str_sha1 = strcmp(typ_val, (char *) SHA1_STR);
				
				if ( str_sha1 == 0)
				{
					ret = vfs_setxattr(lower_path.dentry, (char *) USR_INT_VAL, (void *) BLANK_SHA1, (ssize_t) 2*SHA1_SIGNATURE_SIZE, XATTR_REPLACE);
				}
